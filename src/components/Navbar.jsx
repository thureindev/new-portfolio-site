...
const Navbar = ({ location_path,
    open, setOpen,
    isDarkMode, setIsDarkMode }) => {

    const [navigation, setNavigation] = useState([
        { id: 1, name: 'Home', href: '/', isActive: location_path === '/' ? true : false },
        { id: 2, name: 'Projects', href: '/projects', isActive: location_path === '/projects' ? true : false },
        { id: 3, name: 'Contact', href: '/contact', isActive: location_path === '/contact' ? true : false },
    ]);

    useEffect(() => {
        setNavigation(navigation.map((item) => (item.href === location_path) ? { ...item, isActive: true } : { ...item, isActive: false }));
                        ...
                        <ul
                            ...
                            {/* ----- -----  Link list ----- ----- ----- ----- */
                                navigation.map((item) => (
                                    <li
                                        key={item.id}
                                        className={joinClassNames(item.isActive ? 'active' : '', 'my-4 pl-2 lg:my-0 lg:pl-2 lg:pr-1')}
